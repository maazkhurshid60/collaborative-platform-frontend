...
class ClientApiService {
  ...
  async getAllTotalClient() {
    try {
      const response = await this.api.get(
        `${API_BASE_URL}/client/get-total-clients`,
      );
      return response?.data;
    } catch (error) {
      const errMsg =
        error instanceof Error ? error.message : "Failed to get total client";
      toast.error(errMsg);
    }
  }
  ...
  async getAllClient(loginUserId: string) {
    try {
      const response = await this.api.post(
        `${API_BASE_URL}/client/get-all-clients?limit=1000`,
        { loginUserId },
      );
      return response?.data;
    } catch (error) {
  ...
  async getClientById(id: string) {
    try {
      const response = await this.api.get(
        `${API_BASE_URL}/client/get-client/${id}`,
      );
      return response?.data;
    } catch (error) {
      const errMsg =
  ...
  async deleteClientApi(data: selectedClientIdType) {
    try {
      const response = await this.api.delete(
        `${API_BASE_URL}/client/delete-client`,
        { data: { clientId: data?.clientId, providerId: data?.providerId } },
      );
      return response?.data;
    } catch (error) {
      const errMsg =
  ...
  async updateClientApi(data: FormData) {
    try {
      const response = await this.api.patch(
        `${API_BASE_URL}/client/update-client`,
        data,
        {
          headers: {
  ...
  async addClientApi(data: FormData) {
    const response = await this.api.post(
      `${API_BASE_URL}/client/add-client`,
      data,
      {
        headers: {
          "Content-Type": "multipart/form-data",
  ...
  async addExistingClientToProvider(data: FormData) {
    try {
      const response = await this.api.post(
        `${baseUrl}/client/add-existing-client-to-provider`,
        data,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  async updateExistingClientApi(data: ClientType) {
    try {
      const response = await this.api.patch(
        `${API_BASE_URL}/client/update-existing-client`,
        data,
      );
      return response.data;
    } catch (error: unknown) {
  ...
  async searchUsers(query: string) {
    try {
      const response = await this.api.get(
        `${getApiBaseUrl()}/user/search-users?q=${encodeURIComponent(query)}`,
      );
      return response.data;
    } catch (error) {
      const errMsg =
