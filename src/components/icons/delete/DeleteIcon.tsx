...
const DeleteIcon = ({ disabled = false, tooltipText = "Delete", tooltipPosition, ...props }: DeleteIconProps) => (
    <div
    ...
    >
        ...
        {!disabled && <ToolTip toolTipText={tooltipText} position={tooltipPosition} />}
    </div>
);
...
export const EditIcon = ({ disabled = false, tooltipText = "Delete", tooltipPosition, ...props }: DeleteIconProps) => (
    <div
        className={`relative group ${disabled ? "cursor-not-allowed opacity-50 pointer-events-none" : "cursor-pointer"}`}
    >
