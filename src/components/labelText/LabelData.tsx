...
const LabelData: React.FC<LabelDataProps> = ({ label, data, required = false }) => {
    return (
        <div>
            ...
            <p className={`text-[14px] py-2 font-medium text-textGreyColor ${(label === "Email ID" || label === "Email") ? "lowercase" : (label === "Client ID" || label === "License Number") ? "normal-case" : "capitalize"}`}>{data}</p>
        </div>
    )
}
