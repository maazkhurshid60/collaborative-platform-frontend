...
const BaaModal: React.FC<BaaModalProps> = ({
  ...
  // Replace placeholder {{recipient}}, {{date}}, {{effective_date}}, and Effective Date patterns
  const processedContent = React.useMemo(() => {
    if (!content) return "";
    const name = recipientName?.trim() || "Provider / Covered Entity";
    ...
    return content
      ...
      .replace(/\{\{\s*RECIPIENT\s*\}\}/g, name)
      .replace(/\{\{\s*date\s*\}\}/gi, today)
      .replace(/\{\{\s*effective_date\s*\}\}/gi, today)
      .replace(/\{\{\s*effectiveDate\s*\}\}/gi, today)
      .replace(/\(the\s+["']?Effective Date["']?\)/gi, `on ${today}`);
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-linear-to-r from-[#1a5f6e] to-[#2C9993] rounded-t-2xl shrink-0">
        ...
        {/* Scroll hint banner */}
        {!hasScrolledToBottom ? (
          <div className="shrink-0 bg-amber-50 border-b border-amber-200 px-5 py-2.5 flex items-center gap-2">
            <svg
          ...
          <div className="shrink-0 bg-emerald-50 border-b border-emerald-200 px-5 py-2.5 flex items-center gap-2">
            <svg
        ...
        <div className="shrink-0 border-t border-gray-200 bg-gray-50 rounded-b-2xl px-6 py-5 space-y-4">
          <label
            htmlFor="baa-agree-checkbox"
          ...
          >
            ...
            <span className="text-sm text-gray-700 leading-relaxed select-none">
              ...
              <strong className="text-[#2C9993]">{title}</strong>. I understand
              this agreement is legally binding and required for registration.
            </span>
          </label>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={onCancel}
              className="flex-1 px-4 py-3 rounded-xl border-2 border-gray-200 text-gray-600 font-semibold text-sm hover:border-gray-300 hover:bg-gray-100 transition-all"
            >
              Cancel
            </button>
            <button
              ...
              className={`flex-1 px-4 py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                agreed
                  ? "bg-linear-to-r from-[#1a5f6e] to-[#2C9993] text-white hover:from-[#155260] hover:to-[#238a84] shadow-md hover:shadow-lg active:scale-[0.98]"
