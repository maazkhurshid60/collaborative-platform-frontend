...
const PaymentOverDueModal: React.FC<PaymentOverDueModalProps> = ({ onClose, billingCycle }) => {
    ...
    const handleViewInvoice = async () => {
        try {
            ...
            // Find latest overdue payment or just the latest payment
            const latestPayment = response?.length ? response[0] : null;

            if (latestPayment) {
                // Map to InvoiceModal format
                setInvoiceData({
                    invoiceNo: latestPayment.invoiceNo,
                    date: latestPayment.date,
                    dueDate: "Immediate", // Overdue, so immediate
                    billTo: latestPayment.billTo || {
                    ...
                    notes: "Please make payment immediately to avoid service interruption."

                });
                setShowInvoiceModal(true);
            } else {
                // Fallback if no API data or empty
                setShowInvoiceModal(true);
            }
        } catch (error) {
            console.error("Failed to fetch invoice", error);
            // Still show modal with default/empty state if error
            setShowInvoiceModal(true);
        }
    }
