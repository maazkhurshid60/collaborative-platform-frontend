...
const AddMembersToGroupModalBody: React.FC<AddMembersToGroupModalBodyProps> = ({
  ...
  const {
    ...
    queryKey: ["providers"],
    queryFn: async () => {
      const response = await providerApiService.getAllProviders(
        loginUserId ?? "",
      );
  ...
  const { mutate: addMembers, isPending } = useMutation({
    ...
    onSuccess: (response: any) => {
      ...
      const addedNames: string[] = Array.isArray(data?.addedNames)
        ? data.addedNames
        : [];
  ...
  if (isLoading) return <Loader inline text="Loading providers..." />;
  if (isError)
    return <p className="p-4 text-redColor">Failed to load providers.</p>;

  return (
    <form onSubmit={handleSubmit} className="p-2">
      <p className="text-[13px] text-textGreyColor mb-3">
        Pick providers already on the platform to add to this group. They won't
        receive an email — they'll just see the group appear in their chat list.
        Already-in-group providers are hidden.
      </p>
      ...
      <div className="max-h-80 overflow-y-auto px-2 pt-2 pb-1">
