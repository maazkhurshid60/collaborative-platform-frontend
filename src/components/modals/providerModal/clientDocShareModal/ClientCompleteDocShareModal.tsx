...
const ClientCompleteDocShareModal: React.FC<
  ClientCompleteDocShareModalProps
> = ({ completedDoc, clientId, showDownloadButton = false }) => {
  const [docContent, setDocContent] = useState<string>("");
  const contentRef = useRef<HTMLDivElement>(null);

  const [previewKind, setPreviewKind] = useState<"pdf" | "image" | "docx">(
    "docx",
  );
    ...
    const loadDoc = async () => {
      ...
      if (extension === "pdf") {
        ...
        try {
          ...
          setDocContent(
            '<p style="color:red;">Unable to load document content.</p>',
          );
        }
      } else {
        setPreviewKind("pdf");
  ...
  const rawRecords =
    (completedDoc as any)?.sharedWith ||
    (completedDoc as any)?.sharedRecord ||
    (completedDoc as any)?.sharedRecords ||
    [];
  const filteredRecords = Array.isArray(rawRecords)
    ? rawRecords.filter(
        (r: any) => !clientId || String(r?.clientId) === String(clientId),
      )
    : [];
  ...
  const handleDownloadPDF = async () => {
    if (previewKind === "pdf" && completedDoc?.url) {
      try {
        ...
        anchor.href = url;
        const fileName = completedDoc.name
          ? `${completedDoc.name}_signed.pdf`
          : "completed_document_signed.pdf";
        ...
        console.error(
          "Direct PDF download failed, falling back to opening in a new tab:",
          err,
        );
    ...
    try {
      ...
      if (previewKind === "pdf") {
        ...
        const canvas = await html2canvas(clone, {
          ...
          onclone: (clonedDoc) => {
            const clonedElement =
              clonedDoc.getElementById("complete-doc-clone");
            if (clonedElement) {
    ...
    <ModalLayout
      ...
      modalBodyContent={
        <div className="mt-10 ">
          ...
          <div
            className="max-h-125 overflow-y-auto rounded p-2 bg-white"
            ref={contentRef}
          >
            {/* CONTENT AREA */}
            ...
            <div id="provider-footer-section">
              ...
              {(() => {
                ...
                if (
                  directSignature &&
                  directSignature !== "null" &&
                  !seenUrls.has(directSignature)
                ) {
                ...
                return (
                  <div className="mt-8 border-t border-dashed border-gray-300 pt-6">
                    ...
                    <p className="font-semibold text-[14px] text-gray-700 mb-3">
                      E-Signature{allSigs.length > 1 ? "s" : ""} (
                      {allSigs.length}):
                    </p>
                        ...
                        >
                          <div className="border-2 border-dashed border-gray-300 rounded-lg p-2 bg-white shadow-sm">
                            <img
                              ...
                              className="w-75 h-30 object-contain"
                              onError={(e) => {
              ...
              {filteredRecords.length === 0 && !directSignature && (
                <p className="text-sm text-gray-500 mt-4 italic">
                  No signatures found for this client on this document.
                </p>
              )}
            </div>
          </div>
