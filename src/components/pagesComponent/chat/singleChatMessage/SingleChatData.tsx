...
const SingleChatData: React.FC<SingleChatDataType> = ({ data, onClick, activeId }) => {
    return (
        <div className="">
            ...
            <div className={`pb-2 pt-2 pl-2 flex items-center gap-x-2  hover:bg-primaryColorLight transition-all duration-300 cursor-pointer hover:rounded-md ${activeId === data?.id ? "bg-primaryColorLight rounded-md" : "bg-white"}`} onClick={onClick}>
                {/* <Img size={30} /> */}
                <div className='w-[80%]'>
                    <p className={`font-[Poppins]  text-[14px] text-textColor  ${data?.totalUnread !== 0 ? "font-semibold" : "font-normal "}`}>
                        {data.senderId}
                    </p>
