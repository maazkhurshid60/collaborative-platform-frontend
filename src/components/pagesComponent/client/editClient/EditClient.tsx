...
const EditClient = () => {
  ...
  const isCreatorOrSuperAdmin =
    String(selectedClientData?.createdByProviderId) === String(providerId) ||
    ...
    >
        ...
        <>
          <div className="flex items-center mt-4 md:w-[60%] lg:w-[50%] xl:w-[30%] mb-3">
            {tabData.map((tab, id) => (
              <p
                key={id}
                className={`w-1/2  cursor-pointer text-center transition-colors duration-300 ${
                  activeTab === id
                    ? "bg-primaryColorDark text-white rounded-md px-6 py-2 font-medium"
                    : "font-normal"
                }`}
          ...
          <div className="relative">
            <hr className="text-textGreyColor/30 h-0.5 w-full mt-0 absolute top-0" />
          </div>
        </>
      )}

      {
        ...
        !canEditDetails || activeTab === 1 ? (
          <ShareClientDoc
            clientId={id}
            recipientId={selectedClientData?.userId}
            clientEmail={selectedClientData?.user?.email}
          />
        ) : (
