      ...
      clientData?.filter(
        (client: ClientType) =>
          client?.providerList?.some(
            (provider) =>
              provider?.provider?.user?.id === loginUserId?.user?.id,
          ) ||
    ...
    setModalConfig({
      ...
      text: isDetach ? (
        <div className="text-black">
          ...
          be removed from your list, but their account will remain active.
        </div>
      ) : (
        <div className="text-black">
          By deleting this account you won’t be able to track record of your
                  ...
                  >
                    ...
                    <td className="px-4 py-4 capitalize">
                      {data?.user?.gender === "PREFER_NOT_TO_SAY"
                        ? "Prefer not to say"
                        : String(data?.user?.gender).toLocaleLowerCase()}
                    </td>
                    <td className="px-4 py-4 lowercase">{data?.user?.email}</td>
                    <td className="px-4 py-4">
                      <span
                        className={`px-2 py-1 rounded text-xs  ${data?.user?.status?.toLowerCase() === "active" ? " text-primaryColorDark" : " text-red-800"}`}
                      >
                      ...
                      data?.providerList === undefined ? (
                        ...
                        <>
                          {data?.providerList
                            ...
                            ?.sort((a, b) => {
                              const isA =
                                a?.provider?.user?.id === loginUserId?.user?.id;
                              const isB =
                                b?.provider?.user?.id === loginUserId?.user?.id;
