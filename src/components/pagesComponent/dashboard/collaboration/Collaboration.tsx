...
const Collaboration = () => {
  ...
  const unBlockProviders = useMemo(() => {
    return allChannels?.sort((a: ChatChannelType, b: ChatChannelType) => {
  ...
  const filteredChats = searchText
    ? unBlockProviders?.filter((channel: ChatChannelType) => {
        const recipient =
          channel?.providerAId === loginUserUserId
            ? channel?.providerB
            : channel?.providerA;
  ...
  const filteredGroups = useMemo(() => {
    ...
    return (
      groups?.sort((a: GroupChat, b: GroupChat) => {
        const aTime = new Date(
          a?.lastMessage?.createdAt || a?.updatedAt || 0,
        ).getTime();
        const bTime = new Date(
  ...
  return (
    <>
      ...
      <div className="relative">
        <hr className="text-textGreyColor/30 h-px w-full mt-2" />
        <hr
          className={`text-primaryColorDark h-px w-1/2 mt-2 absolute -top-2 transition-all duration-300 ease-in-out ${activeTab === 0 ? "left-0" : "left-1/2"}`}
        />
      </div>
      <div className="mt-4">
        {activeTab === 0 ? (
              ...
              <Groups
                ...
                onClick={() => {
                  ...
                  messageApiService
                    ...
                    .catch((err) => {
                      console.error(
                        "Failed to mark group messages as read",
                        err,
                      );
                    });
                }}
