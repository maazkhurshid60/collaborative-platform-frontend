...
const DashboardLayout = () => {
  ...
  const isRefreshing = useSelector(
    (state: RootState) => state.LoginUserDetail?.isRefreshing,
  );
  ...
  return (
    <div className=" h-screen flex ">
      ...
      {token && !isPaymentFlow && (
        <div
          className={`
                    ...
                    ${isSideBarClose ? "left-0" : "-left-250"} 
                    md:left-0 
                   `}
        >
          <Sidebar />
      ...
      {/* Content area */}
      <div
        className={`grow bg-gray-100 overflow-auto ${token ? "w-[80%]" : "w-full"}`}
      >
        ...
        <>
          ...
          {showSubscriptionCanceled && (
            <SubscriptionCanceledModal
              onClose={() => setShowSubscriptionCanceled(false)}
            />
          )}
        </>
      )}
