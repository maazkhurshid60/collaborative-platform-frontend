...
import { VideoOff, UserCheck, PhoneCall, Video } from "lucide-react";
...
interface CallRoomVideoGridProps {
  ...
  pendingMediaRequest?: { fromName: string } | null;
  onRespondMediaRequest?: (accepted: boolean) => void;
}
...
export const CallRoomVideoGrid: React.FC<CallRoomVideoGridProps> = ({
  ...
  onApproveGuest,
  pendingMediaRequest,
  onRespondMediaRequest,
}) => {
  const mediaRequestBanner = pendingMediaRequest && onRespondMediaRequest && (
    <div className="mb-4 flex items-center justify-between rounded-xl bg-blue-50 p-3 text-xs text-blue-800 border border-blue-200">
      <div className="flex items-center gap-2">
        <Video size={16} />
        <span>{pendingMediaRequest.fromName} wants to turn on video</span>
      </div>
      <div className="flex gap-2">
        <button
          type="button"
          onClick={() => onRespondMediaRequest(false)}
        ...
        <button
          type="button"
          onClick={() => onRespondMediaRequest(true)}
          className="rounded-full bg-blue-600 px-3 py-1 text-[11px] font-semibold text-white shadow-2xs hover:bg-blue-700 cursor-pointer"
        >
          Accept
        </button>
      </div>
      ...
      <div className="relative rounded-2xl bg-gray-950 p-3 min-h-90">
        <div className="relative flex min-h-90 items-center justify-center rounded-xl bg-gray-900 overflow-hidden">
          ...
          <video
            ...
            autoPlay
            playsInline
            className={`h-full w-full object-cover ${isConnectingOrRinging || peerCameraOff ? "hidden" : "block"}`}
          />

          {/* Peer has no active video track (audio-only / camera off) */}
          {!isConnectingOrRinging && peerCameraOff && (
