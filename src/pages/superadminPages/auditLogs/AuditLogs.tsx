...
import { MeetingsAuditSection } from "../meetings/MeetingsAudit";

const AuditLogs = () => {
  const heading = ["User", "Action", "Resource", "Timestamp", "Action"];
  const [activeTab, setActiveTab] = useState<"system" | "meetings">("system");
  ...
  const filteredLogs = logs.filter(
    (log: any) => log.user?.role !== "superAdmin",
  );
    ...
    <OutletLayout heading="Audit Logs">
      <div className="flex border-b border-gray-200 mb-6 gap-2">
        <button
          ...
          className={`py-2.5 px-5 text-sm font-semibold border-b-2 transition-all cursor-pointer ${
            activeTab === "system"
              ? "border-[#2CB3A8] text-[#2CB3A8] font-bold"
        ...
        >
          Meetings &amp; Call Audit Logs
        </button>
      </div>

      {activeTab === "meetings" ? (
        <MeetingsAuditSection />
      ) : (
        <>
                ...
                <div className="p-4 bg-gray-50 rounded-lg overflow-auto max-h-[60vh]">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="space-y-1">
                      ...
                      <p className="font-semibold text-gray-900">
                        {selectedLog.user?.fullName || "System/Unknown"}
                      </p>
                      <p className="text-sm text-gray-600">
                        {selectedLog.user?.email || "No email provided"}
                      </p>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-gray-500">
                        Resource ID: {selectedLog.resourceId || "None"}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">
                  ...
                  <div className="mt-4 border-t pt-4">
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">
                      Technical Details (JSON Metadata)
                    </p>
              ...
              <>
                  ...
                  heading={[
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setSelectedIds(
                            filteredLogs.map((log: any) => log.id),
                          );
                        } else {
                          setSelectedIds([]);
                        }
                ...
                >
                  {filteredLogs.map((log: any) => (
                    <tr
                      key={log.id}
                      className="border-b border-b-solid border-b-lightGreyColor pb-4 hover:bg-gray-50 transition-colors"
                    >
