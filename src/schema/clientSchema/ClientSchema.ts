...
export const accountSchema = z.object({
  ...
  licenseNo: z.string().min(1, "License number is required")
    .regex(/(?=.*[a-z])/, "License number must contain at least one lowercase letter")
    .regex(/(?=.*[A-Z])/, "License number must contain at least one uppercase letter")
    .regex(/(?=.*\d)/, "License number must contain at least one number")
    .regex(/(?=.*[\W_])/, "License number must contain at least one special character"),
