...
export const appointmentApiService = {
  ...
  startInstantCall: async (payload: {
    ...
    const response = await axiosInstance.post(
      `/appointments/start-instant-call`,
      payload,
    );
    return response.data;
  },
  getPublicAvailableSlots: async (slug: string, from?: string, to?: string) => {
    const response = await axiosInstance.get(
      `/appointments/public/${slug}/available-slots`,
      {
        params: { from, to },
      },
    );
  ...
  acceptMyAppointment: async (appointmentId: string) => {
    const response = await axiosInstance.patch(
      `/appointments/me/${appointmentId}/accept`,
    );
    return response.data;
  },
  declineMyAppointment: async (appointmentId: string) => {
    const response = await axiosInstance.patch(
      `/appointments/me/${appointmentId}/decline`,
    );
    return response.data;
  },
  rescheduleAppointment: async (
    appointmentId: string,
    data: { newStartTime: string; reason?: string },
  ) => {
    const response = await axiosInstance.patch(
      `/appointments/me/${appointmentId}/reschedule`,
      data,
    );
    return response.data;
  },
  resendAppointmentEmail: async (appointmentId: string) => {
    const response = await axiosInstance.post(
      `/appointments/me/${appointmentId}/resend-email`,
    );
    return response.data;
  },
  getAppointmentShareLink: async (appointmentId: string) => {
    const response = await axiosInstance.get(
      `/appointments/me/${appointmentId}/share-link`,
    );
    return response.data;
  },
  getCallJoinInfo: async (appointmentId: string) => {
    const response = await axiosInstance.get(
      `/appointments/me/${appointmentId}/call-join`,
    );
    return response.data;
  },
  ...
  deleteSingleCallLog: async (appointmentId: string) => {
    const response = await axiosInstance.delete(
      `/appointments/me/call-logs/${appointmentId}`,
    );
    return response.data;
  },
  bulkDeleteCallLogs: async (appointmentIds: string[]) => {
    const response = await axiosInstance.post(
      `/appointments/me/call-logs/bulk-delete`,
      {
        appointmentIds,
      },
    );
